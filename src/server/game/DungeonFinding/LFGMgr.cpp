...
namespace lfg
{
    LFGDungeonData::LFGDungeonData(LFGDungeonEntry const* dbc)
        : id(dbc->ID),
          name(dbc->Name),
          map(dbc->MapID),
          type(dbc->TypeID),
          ...
          difficulty(Difficulty(dbc->DifficultyID)),
          seasonal((dbc->Flags & LFG_FLAG_SEASONAL) != 0),
          x(0.0f),
          y(0.0f),
          z(0.0f),
          o(0.0f),
          requiredItemLevel(0),
          requiredTanks(dbc->Count_tank),
          requiredHealers(dbc->Count_healer),
    ...
    void LFGMgr::LoadRewards()
    {
        ...
        QueryResult result = WorldDatabase.Query(
            "SELECT dungeonId, maxLevel, firstQuestId, otherQuestId, shortageQuestId, completionsPerPeriod, dailyReset "
            "FROM lfg_dungeon_rewards ORDER BY dungeonId, maxLevel ASC");
        ...
        do
        {
            ...
            {
                TC_LOG_ERROR("sql.sql", "Dungeon %u specified in table `lfg_dungeon_rewards` does not exist!",
                             dungeonId);
                continue;
            }
            ...
            if (!maxLevel || maxLevel > sWorld->getIntConfig(CONFIG_MAX_PLAYER_LEVEL))
            {
                TC_LOG_ERROR("sql.sql",
                             "Level %u specified for dungeon %u in table `lfg_dungeon_rewards` can never be reached!",
                             maxLevel, dungeonId);
            ...
            {
                TC_LOG_ERROR("sql.sql",
                             "First quest %u specified for dungeon %u in table `lfg_dungeon_rewards` does not exist!",
                             firstQuestId, dungeonId);
                continue;
            }
            ...
            {
                TC_LOG_ERROR("sql.sql",
                             "Other quest %u specified for dungeon %u in table `lfg_dungeon_rewards` does not exist!",
                             otherQuestId, dungeonId);
                otherQuestId = 0;
            }
            ...
            if (shortageQuestId && !sObjectMgr->GetQuestTemplate(shortageQuestId))
            {
                TC_LOG_ERROR(
                    "sql.sql",
                    "Shortage quest %u specified for dungeon %u in table `lfg_dungeon_rewards` does not exist!",
                    shortageQuestId, dungeonId);
                shortageQuestId = 0;
            }

            RewardMapStore.insert(LfgRewardContainer::value_type(
                dungeonId, new LfgReward(maxLevel, firstQuestId, otherQuestId, shortageQuestId, completionsPerPeriod,
                                         dailyReset)));
            ++count;
        } while (result->NextRow());

        TC_LOG_INFO("server.loading", ">> Loaded %u lfg dungeon rewards in %u ms", count,
    ...
    LFGDungeonData const* LFGMgr::GetLFGDungeon(uint32 id)
    {
        ...
        if (itr != LfgDungeonStore.end()) return &(itr->second);
    ...
    void LFGMgr::LoadLFGDungeons(bool reload /* = false */)
    {
        ...
        QueryResult result = WorldDatabase.Query(
            "SELECT dungeonId, position_x, position_y, position_z, orientation, requiredItemLevel FROM "
        ...
        do
        {
            ...
            {
                TC_LOG_ERROR("sql.sql", "table `lfg_dungeon_template` contains coordinates for wrong dungeon %u",
                             dungeonId);
                continue;
            }
        ...
        TC_LOG_INFO("server.loading", ">> Loaded %u lfg dungeon templates in %u ms", count,
                    GetMSTimeDiffToNow(oldMSTime));
        ...
        for (LFGDungeonContainer::iterator itr = LfgDungeonStore.begin(); itr != LfgDungeonStore.end(); ++itr)
        {
            ...
            if (dungeon.type != LFG_TYPE_RANDOM && dungeon.x == 0.0f && dungeon.y == 0.0f && dungeon.z == 0.0f)
            {
                ...
                if (!at)
                {
                    TC_LOG_ERROR("sql.sql", "Failed to load dungeon %s (Id: %u), cant find areatrigger for map %u",
                                 dungeon.name.c_str(), dungeon.id, dungeon.map);
                    continue;
                }
        ...
        if (reload) CachedDungeonMapStore.clear();
    }
    ...
    {
        if (!isOptionEnabled(LFG_OPTION_ENABLE_DUNGEON_FINDER | LFG_OPTION_ENABLE_RAID_BROWSER)) return;
        ...
        for (LfgRoleCheckContainer::iterator it = RoleChecksStore.begin(); it != RoleChecksStore.end();)
        {
            ...
            if (currTime < roleCheck.cancelTime) continue;
            roleCheck.state = LFG_ROLECHECK_MISSING_ROLE;

            for (LfgRolesMap::const_iterator itRoles = roleCheck.roles.begin(); itRoles != roleCheck.roles.end();
                 ++itRoles)
            {
        ...
        for (LfgProposalContainer::iterator it = ProposalsStore.begin(); it != ProposalsStore.end();)
        {
            ...
            if (itRemove->second.cancelTime < currTime) RemoveProposal(itRemove, LFG_UPDATETYPE_PROPOSAL_FAILED);
        }
        ...
        for (LfgPlayerBootContainer::iterator it = BootsStore.begin(); it != BootsStore.end();)
        {
            ...
            if (boot.cancelTime < currTime)
            {
                ...
                for (LfgAnswerContainer::const_iterator itVotes = boot.votes.begin(); itVotes != boot.votes.end();
                     ++itVotes)
                {
                    ObjectGuid pguid = itVotes->first;
                    if (pguid != boot.victim) SendLfgBootProposalUpdate(pguid, boot);
                }
        ...
        if (lastProposalId != m_lfgProposalId)
        {
            ...
            for (LfgProposalContainer::const_iterator itProposal = ProposalsStore.find(m_lfgProposalId);
                 itProposal != ProposalsStore.end(); ++itProposal)
            {
                uint32 proposalId = itProposal->first;
                LfgProposal& proposal = ProposalsStore[proposalId];

                ObjectGuid guid;
                for (LfgProposalPlayerContainer::const_iterator itPlayers = proposal.players.begin();
                     itPlayers != proposal.players.end(); ++itPlayers)
                {
                    ...
                    if (ObjectGuid gguid = GetGroup(guid))
                    {
                        ...
                        SendLfgUpdateStatus(
                            guid,
                            LfgUpdateData(LFG_UPDATETYPE_PROPOSAL_BEGIN, GetSelectedDungeons(guid), GetComment(guid)),
                            true);
                    }
                    else
                        SendLfgUpdateStatus(
                ...
                if (proposal.state == LFG_PROPOSAL_SUCCESS) UpdateProposal(proposalId, guid, true);
            }
        }
        ...
        if (m_QueueTimer > LFG_QUEUEUPDATE_INTERVAL)
        {
            ...
            if (isOptionEnabled(LFG_OPTION_ENABLE_SHORTAGE_REWARDS))
            {
                ...
                {
                    if (dungeon->TypeID != LFG_TYPE_RANDOM) continue;
                    ...
                    if (dpsCountAbsolute >= tankCount || healerCount >= tankCount) roleMask |= PLAYER_ROLE_TANK;

                    if (dpsCountAbsolute >= healerCount || tankCount >= healerCount) roleMask |= PLAYER_ROLE_HEALER;

                    if (dpsCountAbsolute < tankCount && dpsCountAbsolute < healerCount) roleMask |= PLAYER_ROLE_DAMAGE;
    ...
    */
    ...
    {
        if (!player || !player->GetSession() || dungeons.empty()) return;
        ...
        else if (grp)
        {
            ...
            {
                uint8 memberCount = 0;
                for (GroupReference* itr = grp->GetFirstMember(); itr != nullptr && joinData.result == LFG_JOIN_OK;
                     itr = itr->next())
                {
        ...
        if (joinData.result == LFG_JOIN_OK)
        {
            ...
            for (LfgDungeonSet::const_iterator it = dungeons.begin();
                 it != dungeons.end() && joinData.result == LFG_JOIN_OK; ++it)
            {
                LfgType type = GetDungeonType(*it);
                switch (type)
                {
                    case LFG_TYPE_RAID:
                        if (isDungeon) joinData.result = LFG_JOIN_MIXED_RAID_DUNGEON;
                        isRaid = true;
                        break;
                    case LFG_TYPE_RANDOM:
                    ...
                    case LFG_TYPE_HEROIC:
                    case LFG_TYPE_DUNGEON:
                        if (isRaid) joinData.result = LFG_JOIN_MIXED_RAID_DUNGEON;
                        isDungeon = true;
                        break;
                    default:
                        joinData.result = LFG_JOIN_DUNGEON_INVALID;
            ...
            if (joinData.result == LFG_JOIN_OK)
            {
                ...
                if (rDungeonId) dungeons = GetDungeonsByRandom(rDungeonId);

                // if we have lockmap then there are no compatible dungeons
                GetCompatibleDungeons(dungeons, players, joinData.lockmap, isContinue);
                if (dungeons.empty()) joinData.result = grp ? LFG_JOIN_INTERNAL_ERROR : LFG_JOIN_NOT_MEET_REQS;
            }
        }
        ...
        if (joinData.result != LFG_JOIN_OK)
        {
            TC_LOG_DEBUG("lfg.join", "%s joining with %u members. Result: %u, Dungeons: %s", guid.ToString().c_str(),
                         grp ? grp->GetMembersCount() : 1, joinData.result, ConcatenateDungeons(dungeons).c_str());
        ...
        if (rDungeonId && !grp && GetShortageRoleMask(rDungeonId) & roles) SetEnligibleForShortageRewards(guid, true);
        ...
        if (grp)  // Begin rolecheck
        {
            ...
            for (GroupReference* itr = grp->GetFirstMember(); itr != nullptr; itr = itr->next())
            {
                if (Player* plrg = itr->GetSource())
                {
                    ...
                    if (!isContinue) SetSelectedDungeons(pguid, dungeons);
                    roleCheck.roles[pguid] = 0;
                    if (!debugNames.empty()) debugNames.append(", ");
        ...
        else  // Add player to queue
        {
            ...
            player->GetSession()->SendLfgUpdateStatus(
                LfgUpdateData(LFG_UPDATETYPE_JOIN_QUEUE_INITIAL, dungeons, comment), false);
            SetState(guid, isRaid ? LFG_STATE_RAIDBROWSER : LFG_STATE_QUEUED);
            player->GetSession()->SendLfgUpdateStatus(
                LfgUpdateData(isRaid ? LFG_UPDATETYPE_JOIN_RAIDBROWSER : LFG_UPDATETYPE_ADDED_TO_QUEUE, dungeons,
                              comment),
                false);
            player->GetSession()->SendLfgJoinResult(joinData);
        ...
        TC_LOG_DEBUG("lfg.join", "%s joined (%s), Members: %s. Dungeons (%u): %s", guid.ToString().c_str(),
                     grp ? "group" : "player", debugNames.c_str(), uint32(dungeons.size()),
                     ConcatenateDungeons(dungeons).c_str());
    }
    ...
    */
    void LFGMgr::LeaveLfg(ObjectGuid guid, bool disconnected)
    {
        ...
        switch (state)
        {
            ...
            {
                lfg::LfgUpdateType type =
                    state == LFG_STATE_QUEUED ? LFG_UPDATETYPE_REMOVED_FROM_QUEUE : LFG_UPDATETYPE_LEAVE_RAIDBROWSER;
                if (gguid)
                {
                    LfgState newState = LFG_STATE_NONE;
                    LfgState oldState = GetOldState(gguid);

                    // Set the new state to LFG_STATE_DUNGEON/LFG_STATE_FINISHED_DUNGEON if the group is already in a
                    // dungeon This is required in case a LFG group vote-kicks a player in a dungeon, queues, then
                    // leaves the queue (maybe to queue later again)
                    if (Group* group = sGroupMgr->GetGroupByGUID(gguid.GetCounter()))
                        if (group->isLFGGroup() && GetDungeon(gguid) &&
                            (oldState == LFG_STATE_DUNGEON || oldState == LFG_STATE_FINISHED_DUNGEON))
                            newState = oldState;
            ...
            case LFG_STATE_ROLECHECK:
                if (gguid) UpdateRoleCheck(gguid);  // No player to update role = LFG_ROLECHECK_ABORTED
                break;
            case LFG_STATE_PROPOSAL:
            {
                ...
                if (it != ProposalsStore.end()) RemoveProposal(it, LFG_UPDATETYPE_PROPOSAL_DECLINED);
                break;
            }
            case LFG_STATE_NONE:
                break;
    ...
    WorldPackets::LFG::RideTicket const* LFGMgr::GetTicket(ObjectGuid guid) const
    {
        ...
        if (itr != PlayersStore.end()) return &itr->second.GetTicket();
    ...
    /**
       ...
       @param[in]     roles Player selected roles
    */
    void LFGMgr::UpdateRoleCheck(ObjectGuid gguid, ObjectGuid guid /* = ObjectGuid::Empty */,
                                 uint8 roles /* = PLAYER_ROLE_NONE */)
    {
        ...
        if (itRoleCheck == RoleChecksStore.end()) return;
        ...
        if (!dungeons.empty()) dungeon = sLFGMgr->GetLFGDungeon(*dungeons.begin());
        ...
        else
        {
            ...
            while (itRoles != roleCheck.roles.end() && itRoles->second != PLAYER_ROLE_NONE) ++itRoles;

            if (itRoles == roleCheck.roles.end())
            {
                // use temporal var to check roles, CheckGroupRoles modifies the roles
                check_roles = roleCheck.roles;
                roleCheck.state = (dungeon && CheckGroupRoles(check_roles, dungeon)) ? LFG_ROLECHECK_FINISHED
        ...
        for (LfgRolesMap::const_iterator it = roleCheck.roles.begin(); it != roleCheck.roles.end(); ++it)
        {
            ...
            if (sendRoleChosen) SendLfgRoleChosen(pguid, guid, roles);
            ...
            switch (roleCheck.state)
            {
                ...
                case LFG_ROLECHECK_FINISHED:
                    ...
                    SendLfgUpdateStatus(
                        pguid, LfgUpdateData(LFG_UPDATETYPE_ADDED_TO_QUEUE, dungeons, GetComment(pguid)), true);
                    break;
                default:
                    if (roleCheck.leader == pguid) SendLfgJoinResult(pguid, joinData);
                                       ...
                                       bool isContinue)
    {
        ...
        for (GuidSet::const_iterator it = players.begin(); it != players.end() && !dungeons.empty(); ++it)
        {
            ...
            for (LfgLockMap::const_iterator it2 = cachedLockMap.begin();
                 it2 != cachedLockMap.end() && !dungeons.empty(); ++it2)
            {
                ...
                if (itDungeon != dungeons.end())
                {
                    ...
                    if (it2->second.lockStatus == LFG_LOCKSTATUS_RAID_LOCKED && isContinue)
                    {
                        ...
                        ASSERT(dungeon);
                        ASSERT(player);
                        if (InstancePlayerBind* playerBind =
                                player->GetBoundInstance(dungeon->map, Difficulty(dungeon->difficulty)))
                        {
                            if (InstanceSave* playerSave = playerBind->save)
                            {
                                ...
                                if (itLockedDungeon == lockedDungeons.end() ||
                                    itLockedDungeon->second == dungeonInstanceId)
                                    eraseDungeon = false;
                    ...
                    if (eraseDungeon) dungeons.erase(itDungeon);

                    lockMap[guid][dungeonId] = it2->second;
                }
            }
        }
        if (!dungeons.empty()) lockMap.clear();
    }
    ...
    */
    bool LFGMgr::CheckGroupRoles(LfgRolesMap& groles, LFGDungeonData const* dungeon)
    {
        ...
        for (LfgRolesMap::iterator it = groles.begin(); it != groles.end(); ++it)
        {
            ...
            if (role == PLAYER_ROLE_NONE) return false;
            ...
            if (role & PLAYER_ROLE_DAMAGE)
            {
                if (role != PLAYER_ROLE_DAMAGE)
                {
                    ...
                    if (CheckGroupRoles(groles, dungeon)) return true;
                    it->second += PLAYER_ROLE_DAMAGE;
                }
    ...
    */
    void LFGMgr::MakeNewGroup(LfgProposal const& proposal)
    {
        ...
        for (LfgProposalPlayerContainer::const_iterator it = proposal.players.begin(); it != proposal.players.end();
             ++it)
        {
        ...
        for (GuidList::const_iterator it = players.begin(); it != players.end(); ++it)
        {
            ...
            if (group && group != grp) group->RemoveMember(player->GetGUID());
        ...
        for (GuidList::const_iterator it = playersToTeleport.begin(); it != playersToTeleport.end(); ++it)
            if (Player* player = ObjectAccessor::FindPlayer(*it)) TeleportPlayer(player, false);
    }
    ...
    */
    void LFGMgr::UpdateProposal(uint32 proposalId, ObjectGuid guid, bool accept)
    {
        ...
        if (itProposal == ProposalsStore.end()) return;

        LfgProposal& proposal = itProposal->second;

        // Check if proposal have the current player
        LfgProposalPlayerContainer::iterator itProposalPlayer = proposal.players.find(guid);
        if (itProposalPlayer == proposal.players.end()) return;

        LfgProposalPlayer& player = itProposalPlayer->second;
        player.accept = LfgAnswer(accept);

        TC_LOG_DEBUG("lfg.proposal.update", "%s, Proposal %u, Selection: %u", guid.ToString().c_str(), proposalId,
                     accept);
        if (!accept)
        {
        ...
        LfgUpdateData updateData =
            LfgUpdateData(LFG_UPDATETYPE_GROUP_FOUND, GetSelectedDungeons(guid), GetComment(guid));
        for (LfgProposalPlayerContainer::const_iterator it = proposal.players.begin(); it != proposal.players.end();
             ++it)
        {
            ObjectGuid pguid = it->first;
            ...
            int32 waitTime = -1;
            if (sendUpdate) SendLfgUpdateProposal(pguid, proposal);
    ...
    /**
       Remove a proposal from the pool, remove the group that didn't accept (if needed) and readd the other members to
       the queue
    ...
    void LFGMgr::RemoveProposal(LfgProposalContainer::iterator itProposal, LfgUpdateType type)
    {
        ...
        if (type == LFG_UPDATETYPE_PROPOSAL_FAILED)
            for (LfgProposalPlayerContainer::iterator it = proposal.players.begin(); it != proposal.players.end(); ++it)
                if (it->second.accept == LFG_ANSWER_PENDING) it->second.accept = LFG_ANSWER_DENY;

        // Mark players/groups to be removed
        GuidSet toRemove;
        for (LfgProposalPlayerContainer::iterator it = proposal.players.begin(); it != proposal.players.end(); ++it)
        {
            if (it->second.accept == LFG_ANSWER_AGREE) continue;
             ...
             ++it)
        {
            ...
            if (toRemove.find(gguid) !=
                toRemove.end())  // Didn't accept or in same group that someone that didn't accept
            {
                LfgUpdateData updateData;
                if (it->second.accept == LFG_ANSWER_DENY)
                {
                    updateData.updateType = type;
                    TC_LOG_DEBUG("lfg.proposal.remove", "%s didn't accept. Removing from queue and compatible cache",
                                 guid.ToString().c_str());
                }
                else
                {
                    ...
                    TC_LOG_DEBUG(
                        "lfg.proposal.remove",
                        "%s in same group that someone that didn't accept. Removing from queue and compatible cache",
                        guid.ToString().c_str());
                }
            ...
            else
            {
                ...
                if (gguid != guid)
                {
                    SetState(gguid, LFG_STATE_QUEUED);
                    SendLfgUpdateStatus(
                        guid, LfgUpdateData(LFG_UPDATETYPE_ADDED_TO_QUEUE, GetSelectedDungeons(guid), GetComment(guid)),
                        true);
                }
                else
                    SendLfgUpdateStatus(
    ...
    */
    void LFGMgr::UpdateBoot(ObjectGuid guid, bool accept)
    {
        ...
        if (itBoot == BootsStore.end()) return;
        ...
        for (LfgAnswerContainer::const_iterator itVotes = boot.votes.begin(); itVotes != boot.votes.end(); ++itVotes)
        {
            ...
            {
                ++votesNum;
                if (itVotes->second == LFG_ANSWER_AGREE) ++agreeNum;
            }
        }

        bool raidFinderGroup = false;
        if (Group* group = sGroupMgr->GetGroupByGUID(gguid.GetCounter()))
            if (group->isLFRGroup()) raidFinderGroup = true;
        ...
        if (agreeNum ==
            (raidFinderGroup ? LFR_GROUP_KICK_VOTES_NEEDED : LFG_GROUP_KICK_VOTES_NEEDED))  // Vote passed - Kick player
        {
            if (Group* group = sGroupMgr->GetGroupByGUID(gguid.GetCounter()))
    ...
    */
    void LFGMgr::TeleportPlayer(Player* player, bool out, bool fromOpcode /*= false*/, bool saveEntryPoint /*= true*/)
    {
        ...
        if (group && group->isLFGGroup()) dungeon = GetLFGDungeon(GetDungeon(group->GetGUID()));
        ...
        if (out)
        {
            ...
            if (player->GetMapId() == uint32(dungeon->map)) player->TeleportToBGEntryPoint();
        ...
        else if (player->GetMapId() != uint32(dungeon->map))  // Do not teleport players in dungeon to the entrance
        {
            ...
            if (!player->GetMap()->IsDungeon() && saveEntryPoint) player->SetBattlegroundEntryPoint();
            ...
            if (!player->TeleportTo(mapid, x, y, z, orientation)) error = LFG_TELEPORT_RESULT_NO_RETURN_LOCATION;
        }
        else
        {
        ...
        if (error != LFG_TELEPORT_RESULT_NONE) player->GetSession()->SendLfgTeleportError(error);

        TC_LOG_DEBUG("lfg.teleport",
                     "Player %s is being teleported in to map %u "
                     "(x: %f, y: %f, z: %f) Result: %u",
                     player->GetName().c_str(), dungeon->map, dungeon->x, dungeon->y, dungeon->z, error);
    }
    ...
    */
    void LFGMgr::FinishDungeon(ObjectGuid gguid, const uint32 dungeonId, Map const* currMap)
    {
        ...
        {
            TC_LOG_DEBUG("lfg.dungeon.finish", "Group %s finished dungeon %u but queued for %u",
                         gguid.ToString().c_str(), dungeonId, gDungeonId);
            return;
        }
        ...
        for (GuidSet::const_iterator it = players.begin(); it != players.end(); ++it)
        {
            ...
            {
                TC_LOG_DEBUG("lfg.dungeon.finish", "Group: %s, Player: %s already rewarded", gguid.ToString().c_str(),
                             guid.ToString().c_str());
                continue;
            }
            ...
            if (!dungeons.empty()) rDungeonId = (*dungeons.begin());

            SetState(guid, LFG_STATE_FINISHED_DUNGEON);
            SendLfgUpdateStatus(
                guid, LfgUpdateData(LFG_UPDATETYPE_DUNGEON_FINISHED, GetSelectedDungeons(guid), GetComment(guid)),
                true);
            ...
            {
                TC_LOG_DEBUG("lfg.dungeon.finish", "Group: %s, Player: %s dungeon %u is not random or seasonal",
                             gguid.ToString().c_str(), guid.ToString().c_str(), rDungeonId);
                continue;
            }
            ...
            if (!player)
            {
                TC_LOG_DEBUG("lfg.dungeon.finish", "Group: %s, Player: %s not found in world", gguid.ToString().c_str(),
                             guid.ToString().c_str());
                continue;
            }
            ...
            {
                TC_LOG_DEBUG("lfg.dungeon.finish", "Group: %s, Player: %s is in a different map",
                             gguid.ToString().c_str(), guid.ToString().c_str());
                continue;
            }
            ...
            {
                TC_LOG_DEBUG("lfg.dungeon.finish",
                             "Group: %s, Player: %s is in map %u and should be in %u to get reward",
                             gguid.ToString().c_str(), guid.ToString().c_str(), player->GetMapId(), mapId);
                continue;
            }
            ...
            // CanRewardQuest to check currency caps, SatisfyFirstLFGReward to check weekly/daily reward caps for first
            // quest
            if (player->CanRewardQuest(quest, false) &&
                player->SatisfyFirstLFGReward(rDungeonId, reward->completionsPerPeriod))
            {
                if (reward->completionsPerPeriod) player->SetLFGRewardStatus(rDungeonId, reward->dailyReset);
            ...
            // Give rewards
            TC_LOG_DEBUG("lfg.dungeon.finish", "Group: %s, Player: %s done dungeon %u, %s previously done.",
                         gguid.ToString().c_str(), guid.ToString().c_str(), GetDungeon(gguid),
                         !firstReward ? " " : " not");
            LfgPlayerRewardData data =
                LfgPlayerRewardData(dungeon->Entry(), GetDungeon(gguid, false), !firstReward, quest, shortageQuest);
    ...
    */
    LfgReward const* LFGMgr::GetRandomDungeonReward(uint32 dungeon, uint8 level)
    {
        ...
        for (LfgRewardContainer::const_iterator itr = bounds.first; itr != bounds.second; ++itr)
        {
            ...
            if (itr->second->maxLevel >= level) break;
        }
    ...
    */
    ...
    {
        LFGDungeonData const* dungeon = GetLFGDungeon(dungeonId);
        if (!dungeon) return LFG_TYPE_NONE;
    ...
    LfgState LFGMgr::GetState(ObjectGuid guid)
    {
        ...
        if (guid.IsGroup())
        {
            ...
            TC_LOG_TRACE("lfg.data.group.state.get", "Group: %s, State: %s", guid.ToString().c_str(),
                         GetStateString(state).c_str());
        }
        else
        {
            state = PlayersStore[guid].GetState();
            TC_LOG_TRACE("lfg.data.player.state.get", "Player: %s, State: %s", guid.ToString().c_str(),
                         GetStateString(state).c_str());
        }
    ...
    uint32 LFGMgr::GetDungeon(ObjectGuid guid, bool asId /*= true */)
    {
        ...
        TC_LOG_TRACE("lfg.data.group.dungeon.get", "Group: %s, asId: %u, Dungeon: %u", guid.ToString().c_str(), asId,
                     dungeon);
        return dungeon;
    }
    ...
    uint32 LFGMgr::GetDungeonMapId(ObjectGuid guid)
    {
        ...
        uint32 mapId = 0;
        if (dungeonId)
            if (LFGDungeonData const* dungeon = GetLFGDungeon(dungeonId)) mapId = dungeon->map;

        TC_LOG_TRACE("lfg.data.group.dungeon.map", "Group: %s, MapId: %u (DungeonId: %u)", guid.ToString().c_str(),
                     mapId, dungeonId);
        ...
        if (LFGDungeonData const* baseDungeon = GetLFGDungeon(dungeonId))
        {
            ...
            for (LfgDungeonSet::const_iterator it = dungeons.begin(); it != dungeons.end(); ++it)
                if (LFGDungeonData const* dungeon = GetLFGDungeon(*it))
                    if (dungeon->map == mapId && Difficulty(dungeon->difficulty) == difficulty) return dungeon->id;
        }
        return 0;
    }
    ...
    {
        TC_LOG_TRACE("lfg.data.player.comment.get", "Player: %s, Comment: %s", guid.ToString().c_str(),
                     PlayersStore[guid].GetComment().c_str());
        return PlayersStore[guid].GetComment();
    }

    LfgDungeonSet const& LFGMgr::GetSelectedDungeons(ObjectGuid guid)
    {
        TC_LOG_TRACE("lfg.data.player.dungeons.selected.get", "Player: %s, Selected Dungeons: %s",
                     guid.ToString().c_str(), ConcatenateDungeons(PlayersStore[guid].GetSelectedDungeons()).c_str());
    ...
    LfgLockMap const LFGMgr::GetLockedDungeons(ObjectGuid guid)
    {
        ...
        if (!player)
        {
            TC_LOG_WARN("lfg.data.player.dungeons.locked.get",
                        "Player: %s not ingame while retrieving his LockedDungeons.", guid.ToString().c_str());
            return lock;
        }
        ...
        for (LfgDungeonSet::const_iterator it = dungeons.begin(); it != dungeons.end(); ++it)
        {
            ...
            else if (AccessRequirement const* ar =
            ...
            */
            ...
            if (lockStatus)
                lock[dungeon->Entry()] =
                    LfgLockInfoData(lockStatus, dungeon->requiredItemLevel, player->GetAverageItemLevel());
        }
    ...
    void LFGMgr::SetState(ObjectGuid guid, LfgState state)
    {
        if (guid.IsGroup())
        {
            ...
            TC_LOG_TRACE("lfg.data.group.state.set", "Group: %s, New state: %s, Previous: %s, Old state: %s",
                         guid.ToString().c_str(), GetStateString(state).c_str(),
                         GetStateString(data.GetState()).c_str(), GetStateString(data.GetOldState()).c_str());
    ...
    void LFGMgr::SetVoteKick(ObjectGuid gguid, bool active)
    {
        ...
        TC_LOG_TRACE("lfg.data.group.votekick.set", "Group: %s, New state: %d, Previous: %d", gguid.ToString().c_str(),
                     active, data.IsVoteKickActive());
    ...
    {
        TC_LOG_TRACE("lfg.data.player.comment.set", "Player: %s, Comment: %s", guid.ToString().c_str(),
                     comment.c_str());
    ...
    {
        TC_LOG_TRACE("lfg.data.player.dungeon.selected.set", "Player: %s, Dungeons: %s", guid.ToString().c_str(),
                     ConcatenateDungeons(dungeons).c_str());
    ...
    void LFGMgr::DecreaseKicksLeft(ObjectGuid guid)
    {
        ...
        TC_LOG_TRACE("lfg.data.group.kicksleft.decrease", "Group: %s, Kicks: %u", guid.ToString().c_str(),
                     GroupsStore[guid].GetKicksLeft());
    }
    ...
    void LFGMgr::RemovePlayerData(ObjectGuid guid)
    {
        ...
        if (it != PlayersStore.end()) PlayersStore.erase(it);
    }
    ...
    void LFGMgr::AddPlayerToGroup(ObjectGuid gguid, ObjectGuid guid) { GroupsStore[gguid].AddPlayer(guid); }

    void LFGMgr::SetLeader(ObjectGuid gguid, ObjectGuid leader) { GroupsStore[gguid].SetLeader(leader); }

    void LFGMgr::SetTeam(ObjectGuid guid, uint8 team)
    {
        if (sWorld->getBoolConfig(CONFIG_ALLOW_TWO_SIDE_INTERACTION_GROUP)) team = 0;

        PlayersStore[guid].SetTeam(team);
    }

    ObjectGuid LFGMgr::GetGroup(ObjectGuid guid) { return PlayersStore[guid].GetGroup(); }

    void LFGMgr::SetGroup(ObjectGuid guid, ObjectGuid group) { PlayersStore[guid].SetGroup(group); }

    GuidSet const& LFGMgr::GetPlayers(ObjectGuid guid) { return GroupsStore[guid].GetPlayers(); }

    uint8 LFGMgr::GetPlayerCount(ObjectGuid guid) { return GroupsStore[guid].GetPlayerCount(); }

    ObjectGuid LFGMgr::GetLeader(ObjectGuid guid) { return GroupsStore[guid].GetLeader(); }
    ...
    {
        if (Player* player = ObjectAccessor::FindConnectedPlayer(guid)) player->GetSession()->SendLfgJoinResult(data);
    }
    ...
    {
        if (Player* player = ObjectAccessor::FindConnectedPlayer(guid)) player->GetSession()->SendLfgQueueStatus(data);
    }

    bool LFGMgr::IsLfgGroup(ObjectGuid guid) { return guid && guid.IsGroup() && GroupsStore[guid].IsLfgGroup(); }
    ...
    bool LFGMgr::AllQueued(GuidList const& check)
    {
        ...
        for (GuidList::const_iterator it = check.begin(); it != check.end(); ++it)
        {
            ...
            {
                if (state != LFG_STATE_PROPOSAL)
                    TC_LOG_DEBUG("lfg.allqueued",
                                 "Unexpected state found while trying to form new group. Guid: %s, State: %s",
                                 (*it).ToString().c_str(), GetStateString(state).c_str());
    ...
    time_t LFGMgr::GetQueueJoinTime(ObjectGuid guid)
    {
        ...
        if (itr != QueuesStore.end()) return itr->second.GetJoinTime(guid);
    ...
    bool LFGMgr::isOptionEnabled(uint32 option) { return (m_options & option) != 0; }
    ...
    bool LFGMgr::selectedRandomLfgDungeon(ObjectGuid guid)
    {
        if (GetState(guid) != LFG_STATE_NONE)
        {
            ...
            if (!dungeons.empty())
            {
                ...
                if (dungeon && (dungeon->type == LFG_TYPE_RANDOM || dungeon->seasonal)) return true;
            }
        }
    ...
    {
        if (!guid.IsGroup()) guid = GetGroup(guid);

        if (uint32 dungeonId = GetDungeon(guid, true))
            if (LFGDungeonData const* dungeon = GetLFGDungeon(dungeonId))
                if (uint32(dungeon->map) == map && dungeon->difficulty == difficulty) return true;
    ...
    {
        if (id)
            if (LFGDungeonData const* dungeon = GetLFGDungeon(id)) return dungeon->Entry();
    ...
    LfgDungeonSet LFGMgr::GetRandomAndSeasonalDungeons(uint8 level, uint8 expansion)
    {
        ...
        for (lfg::LFGDungeonContainer::const_iterator itr = LfgDungeonStore.begin(); itr != LfgDungeonStore.end();
             ++itr)
        {
            lfg::LFGDungeonData const& dungeon = itr->second;
            if ((dungeon.type == lfg::LFG_TYPE_RANDOM || dungeon.IsRaid() ||
    ...
    {
        if (MapEntry const* mapInfo = sMapStore.LookupEntry(map)) return mapInfo->IsRaid();
        return false;
    }
    ...
    void LFGMgr::SetShortageRoleMask(uint32 dungeonId, uint8 role) { ShortageRoleMaskStore[dungeonId] = role; }

    uint32 LFGMgr::GetShortageRoleMask(uint32 dungeonId) { return ShortageRoleMaskStore[dungeonId]; }
    ...
    bool LFGMgr::IsSoloLFG() const { return m_isSoloLFG; }
