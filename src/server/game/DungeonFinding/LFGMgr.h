...
namespace lfg
{
    ...
    enum LfgJoinResult
    {
        ...
        LFG_JOIN_NOT_MEET_REQS = 0x1F,   // You do not meet the requirements for the chosen dungeons
                                         // LFG_JOIN_PARTY_NOT_MEET_REQS                 = 6,      // One or more party
                                         // members do not meet the requirements for the chosen dungeons
    ...
    struct LfgUpdateData
    {
        LfgUpdateData(LfgUpdateType _type = LFG_UPDATETYPE_DEFAULT)
            : updateType(_type), state(LFG_STATE_NONE), comment("")
        {
        }
    ...
    {
        LfgQueueStatusData(uint8 _queueId = 0, uint32 _dungeonId = 0, int32 _waitTime = -1, int32 _waitTimeAvg = -1,
                           int32 _waitTimeTank = -1, int32 _waitTimeHealer = -1, int32 _waitTimeDps = -1,
                           uint32 _queuedTime = 0, uint8 _tanks = 0, uint8 _healers = 0, uint8 _dps = 0)
            : queueId(_queueId),
    ...
    struct LfgReward
    {
        LfgReward(uint32 _maxLevel = 0, uint32 _firstQuest = 0, uint32 _otherQuest = 0, uint32 _shortageQuest = 0,
                  uint8 _completionsPerPeriod = 0, bool _dailyReset = 0)
            : maxLevel(_maxLevel),
              firstQuest(_firstQuest),
              otherQuest(_otherQuest),
              shortageQuest(_shortageQuest),
              completionsPerPeriod(_completionsPerPeriod),
    ...
    class TC_GAME_API LFGMgr
    {
       ...
       public:
        ...
        // enter in dg/rails by lfg/lfr when solo
        void ToggleSoloLFG();
        bool IsSoloLFG() const;
       ...
       private:
        ...
        // Shortage System
        ShortageRoleMaskContainer ShortageRoleMaskStore;  ///< Stores the roles that are enligible for additional
                                                          ///< rewards indexed by random dungeonId
