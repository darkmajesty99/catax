...
namespace lfg
{
    ...
    */
    ...
    LfgQueueData::LfgQueueData() : joinTime(GameTime::GetGameTime()) { InitializeGroupSetup(); }
    ...
    void LFGQueue::RemoveFromQueue(ObjectGuid guid)
    {
        ...
        if (itDelete != QueueDataStore.end()) QueueDataStore.erase(itDelete);
    }

    void LFGQueue::AddToNewQueue(ObjectGuid guid) { newToQueueStore.push_back(guid); }

    void LFGQueue::RemoveFromNewQueue(ObjectGuid guid) { newToQueueStore.remove(guid); }

    void LFGQueue::AddToCurrentQueue(ObjectGuid guid) { currentQueueStore.push_back(guid); }

    void LFGQueue::AddToFrontCurrentQueue(ObjectGuid guid) { currentQueueStore.push_front(guid); }

    void LFGQueue::RemoveFromCurrentQueue(ObjectGuid guid) { currentQueueStore.remove(guid); }

    void LFGQueue::AddQueueData(ObjectGuid guid, time_t joinTime, LfgDungeonSet const& dungeons,
    ...
    void LFGQueue::RemoveQueueData(ObjectGuid guid)
    {
        ...
        if (it != QueueDataStore.end()) QueueDataStore.erase(it);
    }
    ...
    */
    void LFGQueue::RemoveFromCompatibles(ObjectGuid guid)
    {
        ...
        for (LfgCompatibleContainer::iterator itNext = CompatibleMapStore.begin(); itNext != CompatibleMapStore.end();)
        {
            ...
            if (std::string::npos != it->first.find(strGuid)) CompatibleMapStore.erase(it);
        }
    }
    ...
    */
    LfgCompatibility LFGQueue::GetCompatibles(std::string const& key)
    {
        ...
        if (itr != CompatibleMapStore.end()) return itr->second.compatibility;
    ...
    {
        LfgCompatibleContainer::iterator itr = CompatibleMapStore.find(key);
        if (itr != CompatibleMapStore.end()) return &(itr->second);
    ...
    uint8 LFGQueue::FindGroups()
    {
        ...
        while (!newToQueueStore.empty())
        {
            ...
            TC_LOG_DEBUG("lfg.queue.match.check.new", "Checking [%s] newToQueue(%u), currentQueue(%u)",
                         frontguid.ToString().c_str(), uint32(newToQueueStore.size()),
                         uint32(currentQueueStore.size()));
    ...
    */
    LfgCompatibility LFGQueue::FindNewGroups(GuidList& check, GuidList& all)
    {
        ...
        TC_LOG_DEBUG("lfg.queue.match.check", "Guids: (%s): %s - all(%s)", GetDetailedMatchRoles(check).c_str(),
                     GetCompatibleString(compatibles), GetDetailedMatchRoles(all).c_str());
        ...
        {
            TC_LOG_DEBUG("lfg.queue.match.check", "Guids: (%s) compatibles (cached) changed from bad states to match",
                         GetDetailedMatchRoles(check).c_str());
            SetCompatibles(strGuids, LFG_COMPATIBLES_MATCH);
            return LFG_COMPATIBLES_MATCH;
        }

        if (compatibles != LFG_COMPATIBLES_WITH_LESS_PLAYERS) return compatibles;
        ...
        while (!all.empty())
        {
            ...
            if (subcompatibility == LFG_COMPATIBLES_MATCH) return LFG_COMPATIBLES_MATCH;
            check.pop_back();
        }
        return compatibles;
    }
    ...
    */
    LfgCompatibility LFGQueue::CheckCompatibility(GuidList check)
    {
        ...
        if (check.empty())
        {
            TC_LOG_DEBUG("lfg.queue.match.compatibility.check", "Guids: (%s): Size wrong - Not compatibles",
                         GetDetailedMatchRoles(check).c_str());
        ...
        if (check.size() > 2)
        {
            ...
            if (child_compatibles < LFG_COMPATIBLES_WITH_LESS_PLAYERS)
            {
                TC_LOG_DEBUG("lfg.queue.match.compatibility.check", "Guids: (%s) child %s not compatibles",
                             strGuids.c_str(), GetDetailedMatchRoles(check).c_str());
        ...
        if (check.size() > 1)
        {
            ...
            for (++itguid; itguid != check.end(); ++itguid)
            {
                ...
                std::set_intersection(proposalDungeons.begin(), proposalDungeons.end(), dungeons.begin(),
                                      dungeons.end(), std::inserter(temporal, temporal.begin()));
        ...
        do
        {
            ...
            {
                TC_LOG_DEBUG("lfg.queue.match.compatibility.check",
                             "LFGQueue::CheckCompatibility: (%s) No compatible dungeons%s", strGuids.c_str(),
                             o.str().c_str());
        ...
        {
            TC_LOG_DEBUG("lfg.queue.match.compatibility.check",
                         "LFGQueue::CheckCompatibility: (%s): Size wrong - Not compatibles", strGuids.c_str());
        ...
        for (GuidList::const_iterator it = check.begin();
             it != check.end() && numLfgGroups < 1 && numPlayers <= dungeon->GetMaxGroupSize(); ++it)
        {
            ObjectGuid guid = *it;
            ...
            if (itQueue == QueueDataStore.end())
            {
                TC_LOG_ERROR("lfg.queue.match.compatibility.check", "Guid: [%s] is not queued but listed as queued!",
                             guid.ToString().c_str());
            ...
            // Store group so we don't need to call Mgr to get it later
            for (LfgRolesMap::const_iterator it2 = itQueue->second.roles.begin(); it2 != itQueue->second.roles.end();
                 ++it2)
            {
            ...
            {
                if (!numLfgGroups) proposal.group = guid;
        ...
        if (numLfgGroups > 1)
        {
            TC_LOG_DEBUG("lfg.queue.match.compatibility.check", "Guids: (%s) More than one Lfggroup (%u)",
                         GetDetailedMatchRoles(check).c_str(), numLfgGroups);
        ...
        {
            TC_LOG_DEBUG("lfg.queue.match.compatibility.check", "Guids: (%s) Too many players (%u)",
                         GetDetailedMatchRoles(check).c_str(), numPlayers);

            SetCompatibles(strGuids, LFG_INCOMPATIBLES_TOO_MUCH_PLAYERS);
            return LFG_INCOMPATIBLES_TOO_MUCH_PLAYERS;
        }

        // If it's single group no need to check for duplicate players,
        // ignores, bad roles or bad dungeons as it's been checked before joining
        if (check.size() > 1)
        {
            for (GuidList::const_iterator it = check.begin(); it != check.end(); ++it)
            {
                ...
                for (LfgRolesMap::const_iterator itRoles = roles.begin(); itRoles != roles.end(); ++itRoles)
                {
                    ...
                    for (itPlayer = proposalRoles.begin(); itPlayer != proposalRoles.end(); ++itPlayer)
                    {
                        ...
                        {
                            TC_LOG_ERROR("lfg.queue.match.compatibility.check",
                                         "Guids: ERROR! Player multiple times in queue! [%s]",
                                         itRoles->first.ToString().c_str());
                        }
                    ...
                    if (itPlayer == proposalRoles.end()) proposalRoles[itRoles->first] = itRoles->second;
                }
            }
            ...
            {
                TC_LOG_DEBUG("lfg.queue.match.compatibility.check",
                             "Guids: (%s) not compatible, %u players are ignoring each other",
                             GetDetailedMatchRoles(check).c_str(), playersize);
            ...
            if (!LFGMgr::CheckGroupRoles(proposalRoles, dungeon))
            {
                ...
                TC_LOG_DEBUG("lfg.queue.match.compatibility.check", "Guids: (%s) Roles not compatible%s",
                             GetDetailedMatchRoles(check).c_str(), o.str().c_str());
        ...
        //
        // Normal LFG:
        //   Requires the dungeon's maximum group size.
        //
        // Solo LFG:
        //   A partial group is allowed to create a proposal.
        //
        if (!sLFGMgr->IsSoloLFG() && numPlayers != dungeon->GetMaxGroupSize())
        {
            TC_LOG_DEBUG("lfg.queue.match.compatibility.check", "Guids: (%s) Compatibles but not enough players(%u)",
                         GetDetailedMatchRoles(check).c_str(), numPlayers);
        ...
        {
            TC_LOG_DEBUG("lfg.queue.match.compatibility.check", "Guids: (%s) Group MATCH but can't create proposal!",
                         GetDetailedMatchRoles(check).c_str());
        ...
        for (LfgRolesMap::const_iterator itRoles = proposalRoles.begin(); itRoles != proposalRoles.end(); ++itRoles)
        {
            ...
            {
                if (!leader || !proposal.leader || urand(0, 1)) proposal.leader = itRoles->first;
        ...
        // Mark proposal members as not queued
        // (but don't remove queue data)
        ...
        TC_LOG_DEBUG("lfg.queue.match.compatibility.check", "Guids: (%s) MATCH! Group formed",
                     GetDetailedMatchRoles(check).c_str());
    ...
    void LFGQueue::UpdateQueueTimers(uint8 queueId, time_t currTime, LfgQueueRoleContainer& rolesPerDungeonId)
    {
             ...
             ++itQueue)
        {
            ...
            for (LfgRolesMap::const_iterator itPlayer = queueinfo.roles.begin(); itPlayer != queueinfo.roles.end();
                 ++itPlayer)
                role |= itPlayer->second;
            role &= ~PLAYER_ROLE_LEADER;

            if (role & PLAYER_ROLE_TANK) rolesPerDungeonId[dungeonId].currentTanks++;
            if (role & PLAYER_ROLE_HEALER) rolesPerDungeonId[dungeonId].currentHealers++;
            if (role & PLAYER_ROLE_DAMAGE) rolesPerDungeonId[dungeonId].currentDps++;
            ...
            if (queueinfo.bestCompatible.empty()) FindBestCompatibleInQueue(itQueue);

            LfgQueueStatusData queueData(queueId, dungeonId, waitTime, wtAvg, wtTank, wtHealer, wtDps, queuedTime,
                                         queueinfo.tanks, queueinfo.healers, queueinfo.dps);
            for (LfgRolesMap::const_iterator itPlayer = queueinfo.roles.begin(); itPlayer != queueinfo.roles.end();
                 ++itPlayer)
            {
    ...
    time_t LFGQueue::GetJoinTime(ObjectGuid guid) const
    {
        ...
        if (itr != QueueDataStore.end()) return itr->second.joinTime;
    ...
    std::string LFGQueue::DumpCompatibleInfo(bool full /* = false */) const
    {
        ...
        if (full)
            for (LfgCompatibleContainer::const_iterator itr = CompatibleMapStore.begin();
                 itr != CompatibleMapStore.end(); ++itr)
            {
                ...
                if (!itr->second.roles.empty())
                {
                    ...
                    {
                        if (!first) o << "|";
                        o << role.first.GetRawValue() << " "
                          << GetRolesString(role.second & uint8(~PLAYER_ROLE_LEADER));
                        first = false;
                    }
                    o << ")";
                }
    ...
    void LFGQueue::FindBestCompatibleInQueue(LfgQueueDataContainer::iterator itrQueue)
    {
        ...
        for (LfgCompatibleContainer::const_iterator itr = CompatibleMapStore.begin(); itr != CompatibleMapStore.end();
             ++itr)
        {
    ...
    void LFGQueue::UpdateBestCompatibleInQueue(LfgQueueDataContainer::iterator itrQueue, std::string const& key,
                                               LfgRolesMap const& roles)
    {
        LfgQueueData& queueData = itrQueue->second;

        uint8 storedSize = queueData.bestCompatible.empty()
                               ? 0
